...
Key lang_choose_key(Lang lang) {
  if (lang == 0) {
    return lang_pick_key(LANG_CHOOSE_EN);
  } 
  else {
    return lang_pick_key(LANG_CHOOSE_RU);
  }
}
...
void lang_synchronize(Lang lang) {
  ...
  #ifdef LANG_CHOOSE_DEFAULT
  Key choose_key = lang_choose_key(lang);
  switch (lang_current_choose) 
  {
  case LANG_CHOOSE_CTRL:
    if (shift_current == 1) {
      unregister_code(KC_LSFT);
      register_code(KC_LCTL);
      register_code(choose_key);
      unregister_code(choose_key);
